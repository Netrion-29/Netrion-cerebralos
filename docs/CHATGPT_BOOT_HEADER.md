...
| LDA engine design (generalised) | ✅ IMPLEMENTED (v1) — PR `tier2/lda-engine-impl-v1`; SourceType `LDA` + `LDAEpisode` dataclass; `build_lda_episodes()` builder; 4 gate types in engine with `ENABLE_LDA_GATES` flag (default False); optional LDA gates wired into E05/E06/E21 (`required: false`); contract updated; 37 dedicated tests; 0 NTDS outcome deltas (Roadmap §3 item 16) |
