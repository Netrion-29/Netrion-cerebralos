...
| Roadmap sync (through PR #248) | ✅ COMPLETE (PR #218, merged; subsequently updated through PR #248) — merged-PR state references current |
...
| LDA engine design (generalised) | ✅ IMPLEMENTED (v1+text+start/stop+per-event+vent-recall) — PRs `tier2/lda-engine-impl-v1` (merged), `tier2/lda-text-episodes-v1`; SourceType `LDA` + `LDAEpisode` dataclass; `build_lda_episodes()` builder with text-derived flowsheet day-counter extraction; 4 gate types in engine with `ENABLE_LDA_GATES` flag (default False); per-event LDA gates enabled for E05/E06/E21 via runner toggle + rule `required: true` (PRs #244–#246); vent start/stop episode extraction for E21 recall (PR #248); 180+ dedicated tests; 0 NTDS outcome deltas (Roadmap §3 item 16) |
