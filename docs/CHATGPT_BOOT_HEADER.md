...
| NTDS engine hash baseline sync | ✅ COMPLETE (PR #252) — stale `tests/baselines/ntds_engine_hash.txt` updated to match current `cerebralos/ntds_logic/engine.py` MD5; `gate_pr.sh` now reports `Ntds Engine match=True`; zero behavior change |
| Open PRs | none |
