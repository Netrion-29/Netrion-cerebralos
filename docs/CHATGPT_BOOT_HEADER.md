...
| LDA per-event gate enablement (E05/E06/E21) | ✅ COMPLETE (PRs #244–#246) — E05 CAUTI, E06 CLABSI, E21 VAP LDA gates set `required: true`; per-event toggle in runner (`run_all_events.py` set {5, 6, 21}); protected engine.py not modified |
...
| NTDS engine hash baseline sync | ✅ COMPLETE (PR #252) — NTDS engine MD5 baseline in `_regression_phase1_v2.py` (`BASELINE_HASHES["ntds_engine"]`) synced to current `cerebralos/ntds_logic/engine.py`; `gate_pr.sh` now reports `Ntds Engine match=True`; zero behavior change |
| Docs sync (post-#252) | ✅ COMPLETE (PR #253) — NTDS engine baseline path references fixed |
...
| LDA engine design (generalised) | ✅ IMPLEMENTED (v1+text+start/stop+per-event+vent-recall+multi-episode) — PRs `tier2/lda-engine-impl-v1` (merged), `tier2/lda-text-episodes-v1`; SourceType `LDA` + `LDAEpisode` dataclass; `build_lda_episodes()` builder with text-derived flowsheet day-counter extraction; 4 gate types in engine with `ENABLE_LDA_GATES` flag (default False); per-event LDA gates enabled for E05/E06/E21 via runner toggle + rule `required: true` (PRs #244–#246); vent start/stop episode extraction for E21 recall (PR #248); multi-episode start/stop for MECHANICAL_VENTILATOR and ENDOTRACHEAL_TUBE (PR #250); 180+ dedicated tests; 0 NTDS outcome deltas (Roadmap §3 item 16) |
...
| Canonical audit intake log | **Source of truth for finding lifecycle** — lives in Roadmap §3 · Canonical Audit Intake Log section; 16 items (AUD-001 through AUD-016); classification: KEEP NOW / TIGHTEN NEXT / DEFER |
| Repo-hardening findings (AUD-013–016) | 🔍 TRACKED — 4 validated repo-hardening items: (1) feature-contract leak guard staleness, (2) provenance validation strictness mismatch, (3) v3 output drift blind spot, (4) determinism check narrowness. All `TIGHTEN NEXT`; not yet implemented. See Roadmap §3 audit intake log. |
