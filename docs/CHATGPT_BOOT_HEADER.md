...
| LDA engine design (generalised) | 📐 DESIGN COMPLETE — `docs/audits/LDA_ENGINE_DESIGN_v1.md`; supersedes CAUTI doc for engine spec; covers URINARY_CATHETER, CENTRAL_LINE, MECHANICAL_VENTILATOR + others; defines 4 gate types (`lda_duration`, `lda_present_at`, `lda_overlap`, `lda_device_day_count`) + PatientFacts builder + feature-flag migration + testing strategy; **requires engine-change authorization before implementation** (Roadmap §3 item 16) |
...
| Next phase | **Backlog priority:** (1) Tier 2 PROGRESS_NOTE scoping pass, (2) Delirium shift compliance audit script, (3) PMH-aware gate handling (engine-protected), (4) CAUTI engine changes (pending approval, see design doc), (5) Protocol Data Coverage Mapping — see Roadmap §3 |
