...
- **Repo-hardening findings (AUD-013 – AUD-016):** 4 validated repo-hardening items tracked in the Canonical Audit Intake Log — feature-contract leak guard staleness, provenance validation strictness mismatch, v3 output drift blind spot, determinism check narrowness. All classified `TIGHTEN NEXT`; not yet implemented. See Roadmap §3 audit intake log for details.
