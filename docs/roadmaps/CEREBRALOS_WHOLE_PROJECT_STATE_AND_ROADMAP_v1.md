...
| #240 | `71fc20e` | docs(roadmap): sync status after PR #239 |
| #241 | `7d20e8b` | feat(protocol): add deterministic arrival vitals hardening |
| #242 | `4aab94f` | feat(protocol): wire arrival vitals hardening into runtime build pipeline |
...
| PR | State | Notes |
|----|-------|-------|
| #259 | OPEN | Pupil reactivity extraction from nursing flowsheets (`pupil_reactivity_v1`) |
...
| 15 | **Protocol Data Coverage Mapping** | `docs/audits/PROTOCOL_DATA_COVERAGE_MAPPING_v1.md` | Medium | Medium | **Slices A/B/C COMPLETE; coverage matrix refreshed (PR #262).** Refreshed coverage matrix: 79 EXTRACTED (+19), 48 PARTIAL (−9), 87 MISSING (−10), 16 N/A; actionable coverage 59.3% (was 54.7%); fully extracted 36.9% (was 28.0%). Slice A (sex + discharge disposition): ✅ COMPLETE (PRs #222–#225). Slice B (blood product transfusion): ✅ COMPLETE (PRs #229–#231). Slice C (structured labs): ✅ COMPLETE (PRs #226–#228, #232). Ventilator settings: ✅ COMPLETE (PRs #233–#237). GCS components: ✅ COMPLETE (PRs #238–#239). 6/10 original top gaps closed. |
...
| ~~28~~ | ~~**Determinism check broadening**~~ | ~~`_regression_phase1_v2.py`~~ | ~~Medium~~ | ~~Small~~ | **✅ COMPLETE** — Determinism rerun now loops over all 4 gate patients instead of `PATIENTS[0]` only. Per-patient PASS/FAIL output. See AUD-016. |
...
##### Canonical Audit Intake Log (Consolidated)

Single source of truth for all audit findings, gap analysis results, and
...
| AUD-006 | Coverage mapping top-10 gaps | Antibiotic administration (type, time) — open fracture protocol requires abx within 1 hr | TIGHTEN NEXT | Gap table rank #10; MAR section antibiotic rows available | — | Not started | — | 2026-03-16 |
...
All four findings are now ✅ COMPLETE.
