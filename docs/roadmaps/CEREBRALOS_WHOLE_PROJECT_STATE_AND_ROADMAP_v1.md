...
| ~~6~~ | ~~Source alignment (PROGRESS_NOTE + NURSING_NOTE)~~ | ~~Docs/design: per-event allowed_sources vs raw DATA SOURCE hierarchy~~ | ~~High~~ | ~~Medium~~ | **✅ COMPLETE (design doc)** — `docs/audits/SOURCE_ALIGNMENT_AND_GERI_DELIRIUM_v1.md` §1: full 13-SourceType × 21-event matrix, 3-tier recommendation (Tier 1: CONSULT_NOTE + NURSING_NOTE → 7 gates; Tier 2: PROGRESS_NOTE scoping pass; Tier 3: no-change), ED_NOTE/ANESTHESIA_NOTE retroactive justification |
| ~~7~~ | ~~Geriatric delirium nursing shift assessments~~ | ~~E09: shift-based nursing delirium assessments~~ | ~~High~~ | ~~Medium~~ | **✅ COMPLETE (design doc)** — `docs/audits/SOURCE_ALIGNMENT_AND_GERI_DELIRIUM_v1.md` §2: CAM/bCAM prevalence audit (24/39 patients), 4 proposed `delirium_dx` patterns + 4 noise patterns, 2-phase shift compliance design (Phase 1: standalone audit script, Phase 2: engine `compliance_per_shift` gate type) |
